...
def test_cut_docs_docstring_not_found(mock_import_module, mock_pkgutil_iter_modules, capsys):
    ...
    result = list(cut_docs())

    assert not result
    # captured = capsys.readouterr()

    # Ensure the error is logged correctly
    # assert "Doc String Not Found for cogvideo" in captured.out
...
def test_cut_docs_yield_docstrings(mock_import_module, mock_pkgutil_iter_modules):
    ...
    assert any("mock_docstring" in y for y in [x for x in results])


# def test_cut_docs_no_module(mock_import_module, mock_pkgutil_iter_modules, capsys):
...
#     result = list(cut_docs())
#     # assert exc_info.value.msg == ("Simulated module not found")
...
#     # Ensure the error is logged correctly
#     # assert "Module Not Found for allegro" in captured.out
